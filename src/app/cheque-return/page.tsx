...
import { getBills, getSalespersonContacts, getWhatsAppTemplates, patchBillInMemory, addBillsToMemoryOnly, getTodayDMY } from '@/lib/billStore';
import { apiPushSetting, BILL_SELECT_COLUMNS, mapBillFromSupabase } from '@/lib/apiSync';
...
// ── Smart Cheque & Bill Matching Helpers ───────────────────────────────────────
function isChequeMatch(bCheqRaw: string | undefined | null, qRaw: string): boolean {
  if (!bCheqRaw || !qRaw) return false;
  const b = bCheqRaw.trim().toLowerCase();
  const q = qRaw.trim().toLowerCase();
  if (!b || !q) return false;
  if (b === q) return true;

  // Handle multiple cheques in bill (e.g. "123456+789012" or "123456, 789012")
  const subCheques = b.split(/[/+,]/).map(s => s.trim()).filter(Boolean);
  if (subCheques.length > 1) {
    if (subCheques.some(sc => isChequeMatch(sc, q))) return true;
  }

  // Digits comparison (ignores leading zeros e.g. 000683 === 683)
  const bDigits = b.replace(/\D/g, '');
  const qDigits = q.replace(/\D/g, '');
  if (bDigits && qDigits) {
    if (bDigits === qDigits) return true;
    const bNoZeros = bDigits.replace(/^0+/, '');
    const qNoZeros = qDigits.replace(/^0+/, '');
    if (bNoZeros && bNoZeros === qNoZeros) return true;

    // Standard 6-digit cheque padding comparison
    if (bDigits.length <= 6 && qDigits.length <= 6) {
      if (bDigits.padStart(6, '0') === qDigits.padStart(6, '0')) return true;
    }

    // Suffix match for 3+ digits (e.g. typing last digits of cheque)
    if (qDigits.length >= 3 && bDigits.endsWith(qDigits)) return true;
  }
  return false;
}

function isBillNoMatch(bBillNo: string | undefined | null, qRaw: string): boolean {
  if (!bBillNo || !qRaw) return false;
  const b = bBillNo.trim().toUpperCase();
  const q = qRaw.trim().toUpperCase();
  if (!b || !q) return false;
  if (b === q) return true;
  const bClean = b.replace(/^GST[-/]?/i, '');
  const qClean = q.replace(/^GST[-/]?/i, '');
  if (bClean === qClean) return true;
  const bNoZeros = bClean.replace(/^0+/, '');
  const qNoZeros = qClean.replace(/^0+/, '');
  return bNoZeros.length > 0 && bNoZeros === qNoZeros;
}
  ...
  if (b.partPayments && b.partPayments.length > 0) {
    if (cheqNo) {
      const matching = b.partPayments.filter(
        p => p.chequeNo && isChequeMatch(p.chequeNo, cheqNo)
      );
...
export default function ChequeReturnPage() {
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (
        ...
        cheqRef.current &&
        !cheqRef.current.contains(e.target as Node)
      ) {
  ...
  // Live suggestions while typing in search box
  const liveSuggestions = useMemo(() => {
    const q = cheqInput.trim().toLowerCase();
    ...
    const map = new Map<string, { cheqNo: string; bills: typeof allBills; partyName: string; amt: number; bankName: string }>();

    for (const b of allBills) {
      const cNo = (b.chequeNo || '').trim();
      const hasPartChq = b.partPayments?.some(p => p.chequeNo);
      if (!cNo && !hasPartChq) continue;

      const candidates: string[] = [];
      if (cNo) candidates.push(cNo);
      if (b.partPayments) {
        for (const p of b.partPayments) {
          if (p.chequeNo?.trim()) candidates.push(p.chequeNo.trim());
        }
      }

      for (const cand of candidates) {
        const cLower = cand.toLowerCase();
        const cDigits = cand.replace(/\D/g, '');
        const cNoZeros = cDigits.replace(/^0+/, '');

        const match =
          cLower.includes(q) ||
          (qNoZeros.length >= 2 && cNoZeros.includes(qNoZeros)) ||
          isChequeMatch(cand, q) ||
          (b.billNo && stripGST(b.billNo).toLowerCase().includes(q)) ||
        ...
        if (match) {
          ...
          if (!map.has(key)) {
            map.set(key, {
              ...
              amt: billCheqAmt(b, cand),
              bankName: b.bankName || '',
            });
          } else {
            const entry = map.get(key)!;
            if (!entry.bills.some(eb => eb.billNo === b.billNo)) {
              entry.bills.push(b);
              entry.amt += billCheqAmt(b, cand);
            }
          }
        }
      }
  ...
  // ── Smart Cheque lookup (Memory + Supabase Fallback) ──────────────────────
  async function lookupCheque(overrideQ?: string) {
    const rawQ = (overrideQ !== undefined ? overrideQ : cheqInput).trim();
    if (!rawQ) { setPreviewError('Cheque number darj karo.'); return; }
    ...
    try {
      ...
      let matched = allBills.filter(b => {
        // Cheque match on bill
        if (isChequeMatch(b.chequeNo, rawQ)) return true;
        // Cheque match on partPayments
        if (b.partPayments?.some(p => isChequeMatch(p.chequeNo, rawQ))) return true;
        // Bill number match IF this bill actually has a cheque recorded
        if ((b.chequeNo || b.partPayments?.some(p => p.chequeNo)) && isBillNoMatch(b.billNo, rawQ)) return true;
        // Party name exact match if bill has cheque
        if ((b.chequeNo || b.partPayments?.some(p => p.chequeNo)) && (b.partyName || '').toLowerCase() === rawQ.toLowerCase()) return true;
        return false;
      });
      ...
      if (matched.length === 0 && supabase) {
        ...
        const qNoZeros = qDigits.replace(/^0+/, '');
        const qPadded6 = qDigits.length > 0 && qDigits.length <= 6 ? qDigits.padStart(6, '0') : '';
        const cleanBillNo = cleanQ.replace(/^gst[-/]?/i, '');

        const orConditions = [
          `cheque_no.eq.${cleanQ}`,
          `cheque_no.ilike.%${cleanQ}%`,
        ];
        if (qNoZeros && qNoZeros !== cleanQ) {
          orConditions.push(`cheque_no.ilike.%${qNoZeros}%`);
        }
        if (qPadded6 && qPadded6 !== cleanQ) {
          orConditions.push(`cheque_no.eq.${qPadded6}`);
        }
        if (cleanBillNo) {
          orConditions.push(`bill_no.ilike.%${cleanBillNo}%`);
        }
      ...
      if (matched.length === 0) {
        setPreviewError(`Cheque no. "${rawQ}" kisi bhi bill me nahi mila.`);
        return;
      }

      // Determine resolved cheque number
      const first = matched[0];
      const resolvedCheqNo = first.chequeNo?.trim() ||
        first.partPayments?.find(p => isChequeMatch(p.chequeNo, rawQ))?.chequeNo?.trim() ||
        rawQ;

      // Already added in this session
      if (addedCheques.some(c => isChequeMatch(c.cheqNo, resolvedCheqNo))) {
        setPreviewError(`Cheque "${resolvedCheqNo}" already is entry me add hai.`);
        return;
      }

      // Already in active return list
      const alreadyActive = activeEntries.some(e => {
        const eCheqs = e.chequeNo.split('+').map(x => x.trim());
        return eCheqs.some(ec => isChequeMatch(ec, resolvedCheqNo));
      });
      if (alreadyActive) {
        setPreviewError(`Cheque "${resolvedCheqNo}" already active return list me hai.`);
        return;
      }

      // Amount = sum across all matched bills, checking partPayments first
      const amt = matched.reduce((s, b) => s + billCheqAmt(b, resolvedCheqNo), 0);

      const preview: ChequePreview = {
        cheqNo:     resolvedCheqNo,
      ...
      console.error('[cheque-return] lookupCheque error:', err);
      setPreviewError('Search karne me samasya aayi. Dobara try karein.');
    } finally {
      ...
      <div className="pt-12 px-1.5">
        ...
        <div className="bg-card border border-border rounded-xl p-2 mb-1.5 shadow-sm">
          ...
          <div className="flex gap-1.5 mb-1.5">
            {/* Cheque search */}
            <div className="relative flex gap-1 flex-1 min-w-0">
              <input
                ref={cheqRef}
                type="text"
                value={cheqInput}
                onChange={e => {
                  setCheqInput(e.target.value);
                  setPreviewError('');
                ...
                placeholder={addedCheques.length === 0 ? 'CHQ NO / BILL NO...' : 'ADD MORE...'}
              ...
              <button
                ...
                className="h-8 px-2 rounded-lg bg-muted border border-border hover:border-primary flex items-center gap-1 shrink-0 disabled:opacity-60"
              >
                {isSearching ? (
                ...
                <span className="text-[8px] font-black uppercase text-muted-foreground">
                  {isSearching ? 'FIND...' : addedCheques.length === 0 ? 'SRCH' : 'ADD'}
                </span>
              </button>
              ...
              {showSuggestions && liveSuggestions.length > 0 && (
                <div
                  ...
                  className="absolute left-0 top-9 w-full sm:w-[380px] bg-popover border border-border rounded-lg shadow-xl z-50 overflow-hidden divide-y divide-border/40"
                >
                  <div className="px-2 py-1 bg-muted/60 flex items-center justify-between text-[7px] font-black uppercase tracking-wider text-muted-foreground">
                    <span>Matching Cheques ({liveSuggestions.length})</span>
                  ...
                  <div className="max-h-56 overflow-y-auto">
                    {liveSuggestions.map(s => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          lookupCheque(s.cheqNo);
                        }}
                        className="w-full text-left px-2 py-1.5 hover:bg-accent/40 flex items-center justify-between gap-2 transition-colors cursor-pointer"
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            <span className="text-[9px] font-black tracking-widest text-primary bg-primary/10 px-1 py-0.2 rounded border border-primary/20">
                              {s.cheqNo}
                            </span>
                          ...
                          <p className="text-[7.5px] text-muted-foreground font-semibold truncate">
                            Bills: {s.bills.map(b => stripGST(b.billNo)).join(' + ')}
                          </p>
                        </div>
