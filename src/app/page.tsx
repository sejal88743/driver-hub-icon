...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // For owner role: default to 'OWNER' if nothing saved yet
    if (role === 'owner') {
      const savedDriver = sessionStorage.getItem('vitratrack_selected_driver');
      if (!savedDriver) {
        setSelectedDriver('OWNER');
        sessionStorage.setItem('vitratrack_selected_driver', 'OWNER');
        return;
      }
    }
    const savedDriver = sessionStorage.getItem('vitratrack_selected_driver');
  ...
  // Helper to check if currently selected driver (or logged-in role) is Owner or Staff User
  const { isOwnerSelected, isUserSelected, isOwnerOrUserSelected } = useMemo(() => {
    const selUpper = (selectedDriver || '').trim().toUpperCase();
    const curRole = getRole();
    const matchedDriver = drivers.find(drv => (drv.name || '').trim().toUpperCase() === selUpper);
    const isOwner = selUpper === 'OWNER' || selUpper === '👑 OWNER' || matchedDriver?.role === 'owner' || (curRole === 'owner' && !selUpper);
    const isUser = !isOwner && (
      matchedDriver?.role === 'user' ||
      selUpper === 'PRATIXA' ||
      selUpper === 'KHUSHI' ||
      selUpper === 'TARACHAND' ||
      selUpper === 'SEJAL' ||
      (curRole === 'user' && !selUpper)
    );
    return {
      isOwnerSelected: isOwner,
      isUserSelected: isUser,
      isOwnerOrUserSelected: isOwner || isUser,
    };
  ...
  const filteredBillNos = useMemo(() => {
    ...
    if (!selectedDriver && userRole !== 'owner' && userRole !== 'user') return [];
    ...
    for (const b of bills) {
      // Driver filter: When OWNER or USER is selected, show ALL bills!
      // For regular drivers, ONLY find bills belonging to the selected driver!
      if (!isOwnerOrUserSelected) {
  ...
  }, [bills, debouncedQuery, selectedDriver, commissionMocs, isOwnerOrUserSelected]);

  // Live on-demand Supabase search fallback: if memory doesn't have the bill yet, query Supabase directly
  useEffect(() => {
    const q = debouncedQuery.trim();
    const userRole = getRole();
    if (!q || q.length < 2 || filteredBillNos.length > 0 || selectedBillNo || (!selectedDriver && userRole !== 'owner' && userRole !== 'user')) return;
    ...
    const searchSupabase = async () => {
      try {
        ...
        if (!isOwnerOrUserSelected && selectedDriver) {
  ...
  }, [debouncedQuery, filteredBillNos.length, selectedBillNo, selectedDriver, isOwnerOrUserSelected]);
  ...
  function handleBillSelect(bn: string) {
    ...
    if (!selectedDriver && !isOwnerOrUserSelected) {
    ...
    if (!isOwnerOrUserSelected && billDriverUpper !== selDriverUpper) {
    ...
    const hasPriorMoney = ca > 0 || up > 0 || ch > 0 || col > 0;
    const isSpecialLock = _bMode === 'fbr' || _bMode === 'cancel' || _bMode === 'credit';
    const isPaid = hasPriorMoney || isSpecialLock || _bMode === 'paid' || _bMode === 'cash' || _bMode === 'upi' || _bMode === 'cheque' || _bMode === 'split';

    // Fresh uncollected bills (with 0 money collected, not FBR/Credit) NEVER require lock
    const isMoc = isMocBill(bn, bill);
    const requiresLock = !isMoc && (hasPriorMoney || isSpecialLock);
  ...
  const findExactBill = useCallback((num: string) => {
    ...
    if (!selectedDriver && !isOwnerOrUserSelected) return undefined; // Only find bills for selected driver (unless owner/user)
    ...
    const driverBills = bills.filter(b => {
      if (isOwnerOrUserSelected) return true;
    ...
    return match;
  }, [bills, selectedDriver, isOwnerOrUserSelected]);
  ...
  const processVoiceInput = useCallback((rawTranscript: string) => {
    ...
    if (!matched && billNoToUse && !/^\d+$/.test(billNoToUse.trim())) {
      ...
      matched = bills.find(b => {
        if (isOwnerOrUserSelected) {
          return b.partyName && b.partyName.toLowerCase().includes(q);
        }
        const bDriver = (b.driverName || '').trim().toUpperCase();
        return bDriver === selUpper && b.partyName && b.partyName.toLowerCase().includes(q);
      });
    }
  ...
  }, [bills, findExactBill, handleBillSelect, speakText, selectedBillNo, proceedToSave, openCreditLineCutPopup, recDateOverride, dashDate, selectedDriver, refresh, isOwnerOrUserSelected]);
  ...
  async function doSaveFBR(reason: string) {
    if (!selectedBill || (!selectedDriver && !isOwnerOrUserSelected) || saving) return;
    setSaving(true);
    setSaveError(null);
    setShowFbrReasonModal(false);
    const effectiveDriver = (isOwnerOrUserSelected && selectedBill?.driverName)
      ? selectedBill.driverName
      : (selectedDriver || selectedBill?.driverName || '');
  ...
  async function doSave(lcOverride?: number | null, recDateParam?: string | null, discrepancyReason?: string | null) {
    if (saving || !selectedBillNo) return;
    setSaving(true);
    setSaveError(null);
  ...
  function doSaveOverflow(recDateParam?: string | null) {
    if (saving || !selectedBillNo || !selectedBill) return;
    const finalMode = deriveMode(paymentMode);
    const effectiveDriver = (isOwnerOrUserSelected && selectedBill?.driverName)
      ? selectedBill.driverName
      : (selectedDriver || selectedBill?.driverName || '');
  ...
  function handleSaveClick() {
    ...
    if (!selectedBillNo) {
      toast({ title: 'Bill Select Karein', description: 'Pehle koi bill select karein', variant: 'destructive' });
      billInputRef.current?.focus();
      return;
    }

    if (!selectedDriver && !isOwnerOrUserSelected) {
      toast({ title: 'Driver Select Karein', description: 'Pehle driver select karein tabhi entry save hogi', variant: 'destructive' });
      return;
    }

    if (userCannotEditReceivedBill) {
      toast({ title: 'Permission Denied', description: 'Aapke paas is received bill ko edit karne ki permission nahi hai', variant: 'destructive' });
      return;
    }

    const isMoc = isMocBill(selectedBillNo, selectedBill);
    if (!isMoc && selectedBill && !isOwnerOrUserSelected && selectedDriver) {
      const selDriverUpper = selectedDriver.trim().toUpperCase();
      const billDriverUpper = (selectedBill.driverName || '').trim().toUpperCase();
      if (billDriverUpper && billDriverUpper !== selDriverUpper) {
        toast({ title: 'Driver Mismatch', description: `Ye bill ${selectedBill.driverName || 'dusre driver'} ka hai, ${selectedDriver} ka nahi.`, variant: 'destructive' });
        return;
      }
    }

    if (isLockedForSave) {
      toast({ title: 'Bill Locked Hai', description: "Edit karne ke liye 'Unlock (U)' button dabayein", variant: 'destructive' });
      return;
    }

    const hasAnyAmount = totalCollected > 0;
    const isSpecial = paymentMode === 'FBR' || paymentMode === 'Credit' || paymentMode === 'Del Pending' || paymentMode === 'Unpaid';
    if (!hasAnyAmount && !isMoc && !isSpecial && !isChqMetaOnlyEdit) {
      toast({ title: 'Amount Darj Karein', description: 'Cash / GPay / Cheque amount darj karein ya FBR / Credit / Del Pending chunein', variant: 'destructive' });
    ...
    if (Number(chqAmt) > 0) {
      if (!chequeNo.trim()) {
        toast({ title: 'Cheque No Required', description: 'Cheque number darj karein', variant: 'destructive' });
        setSaveError('Cheque No darj karein');
        setTimeout(() => chequeNoRef.current?.focus(), 40);
        return;
      }
      if (isDriverMode) {
        const hasDate = chqDateDD.trim().length >= 2 || (chequeDate && chequeDate.trim().length >= 4);
        if (!hasDate) {
          toast({ title: 'Cheque Date Required', description: 'Cheque date darj karein', variant: 'destructive' });
          setSaveError('Cheque Date compulsory hai!');
          setTimeout(() => chqDateRef.current?.focus(), 40);
          return;
        }
        ...
        if (!bankName.trim()) {
          toast({ title: 'Bank Required', description: 'Kripya Cheque ke liye Bank Name select karein', variant: 'destructive' });
          setSaveError('Kripya Bank select karein');
          setTimeout(() => bankInputRef.current?.focus(), 40);
          return;
        }
      }
    }
...
*Kripya party se is bill ka payment collection coordinate karein.*`;
  ...
  const hasRealMoney = hasCashRec || hasUpiRec || hasChqRec || hasColRec;
  const hasDateRec = !!selectedBill?.paymentDate && selectedBill.paymentDate.trim() !== '' && selectedBill.paymentDate !== '—' && hasRealMoney;
  const isPaidMode = (_selMode === 'paid' || _selMode === 'cash' || _selMode === 'upi' || _selMode === 'cheque' || _selMode === 'split') && hasRealMoney;
  const isFbrBill = (_selMode === 'fbr' || _selMode === 'cancel') && isSelFBR;
  ...
  const isProtectedBill = !!selectedBill && (hasRealMoney || hasDateRec || isPaidMode || isFbrBill || isCreditBill);
  ...
  const hasDriverSelection = !!selectedDriver || isOwnerOrUserSelected;
  const isDriverMatched = isMocCurrent || !selectedBill || isOwnerOrUserSelected ||
    (selectedBill.driverName || '').trim().toUpperCase() === (selectedDriver || '').trim().toUpperCase();

  const isLockedForSave = !isMocCurrent && isProtectedBill && editLocked && hasRealMoney;
  ...
  const isChqMetaOnlyEdit = !!(
    selectedBillNo && hasDriverSelection && !isDriverMode && totalCollected <= 0 &&
    (
  ...
  const isEditLockedChqMeta = !!(
    selectedBillNo && hasDriverSelection && !isDriverMode &&
    editLocked && isProtectedBill &&
    Number(chqAmt) > 0
  );

  const canSave = !saving && !userCannotEditReceivedBill && (
    (selectedBillNo && hasDriverSelection && chqValid &&
              ...
              <div className={cn("bg-card border border-border rounded-2xl px-4 pt-3 pb-4 shadow-sm space-y-3", isDriverMode && "p-2 rounded-xl space-y-2")}>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={saving}
                    className={cn(
                      "font-black uppercase shadow-lg transition-all",
                      canSave
                        ? "bg-primary hover:bg-primary/90 text-primary-foreground shadow-primary/25 cursor-pointer ring-1 ring-primary/40"
                        : "bg-primary/60 text-primary-foreground/90 hover:bg-primary/70 cursor-pointer",
                      isDriverMode ? "w-full h-9 rounded-xl text-[12px] p-0" : "flex-[1.5] h-10 rounded-xl text-[11px] p-0"
                    )}
                    title={canSave ? "Save Entry (Enter / Ctrl+S)" : "Click karke dekhein entry me kya baaki hai"}
                  >
                    {saving ? (
                      ...
                      <>
                        ...
                        {totalCollected > 0 
                          ? `Save ₹${totalCollected.toLocaleString('en-IN')}` 
                          : paymentMode 
                            ? `Save (${paymentMode})` 
                            : 'Save'}
                      </>
